/* We're going to implement the method <b>Print_By_Rating</b> incorrectly,
   by once again being seduced by associative arrays. */
...
/* I'm implementing Print_By_Rating, which uses the associative
   array feature of a map to sort the quarterbacks by their ratings.
   It inserts negative values to that the map will store the quarterbacks
...
int main(int argc, char **argv)
{
  ...
  try {
    if (argc != 2 && argc != 3) throw((string) "usage: qb_3_sort_by_rating_bad file [prompt]");
    filename = argv[1];
